...
export class TMDBClient {
  ...
  async discoverMovies(
    ...
    if (params.voteAverageGte)
      searchParams['vote_average.gte'] = params.voteAverageGte.toString();
    if (params.voteCountGte)
      searchParams['vote_count.gte'] = params.voteCountGte.toString();
    if (params.withoutGenres)
      searchParams.without_genres = params.withoutGenres;

    return this.makeRequest<TMDBDiscoverResponse>(
      '/discover/movie',
      searchParams
    );
  }
    ...
    return {
      ...
      images: {
        poster: movie.poster_path
          ? `${this.config.imageBaseUrl}w1280${movie.poster_path}`
          : null,
        backdrop: movie.backdrop_path
          ? `${this.config.imageBaseUrl}w1280${movie.backdrop_path}`
          : null,
        stills: (images.stills || [])
          .filter(
            (img) =>
              img.iso_639_1 === null || img.iso_639_1 === this.config.language
          )
  ...
  generateMovieFrames(movie: MovieMetadata, count: number = 5): MovieFrame[] {
    ...
    if (stills.length > 0) {
      const selectedStills = this.shuffleArray(stills).slice(
        0,
        Math.min(count, stills.length)
      );

      selectedStills.forEach((imageUrl) => {
        frames.push({
  ...
  async getPopularMoviesForQuiz(count: number = 10): Promise<MovieMetadata[]> {
    ...
    while (movies.length < count && page <= 5) {
      // Limite à 5 pages max
  ...
  hasLandscapeImages(movie: MovieMetadata): boolean {
    ...
    const validStills = movie.images.stills.filter(
      (url) => url.includes('still') || url.includes('backdrop')
    );
