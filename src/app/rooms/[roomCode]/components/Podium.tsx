...
export default function Podium({ 
  ...
  const getPodiumPosition = (index: number) => {
    switch (index) {
      case 0: return { position: '1st', medal: '🥇', height: 'h-24', bg: 'bg-warning' };
      case 1: return { position: '2nd', medal: '🥈', height: 'h-20', bg: 'bg-base-300' };
      case 2: return { position: '3rd', medal: '🥉', height: 'h-16', bg: 'bg-warning/70' };
      default: return { position: '', medal: '', height: 'h-12', bg: 'bg-base-200' };
    }
  };
          ...
          {topThree.map((player, index) => {
            ...
            return (
              <div key={player.id} className="flex flex-col items-center">
                {/* Position du podium */}
                <div className={`${podiumInfo.height} ${podiumInfo.bg} w-20 rounded-t-lg flex items-end justify-center mb-2 relative`}>
                  <div className="text-2xl mb-2">{podiumInfo.medal}</div>
                </div>
                
                {/* Informations du joueur */}
                <div className={`card ${isCurrentPlayer ? 'ring-2 ring-primary' : ''} ${
        ...
        {topThree.length > 0 && (
          <div className="text-center">
            <div className="text-sm opacity-70">
              {topThree[0].name} wins with {topThree[0].score} point{topThree[0].score !== 1 ? 's' : ''}!
            </div>
          </div>
        )}
      </div>
